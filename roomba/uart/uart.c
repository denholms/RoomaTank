...
/*************************************************************************
Function: UART_getTail()
Purpose:  Get the value for the tail to know how many bytes have been received
Input:    Type (0 for read - RX, 1 for transmit - TX), uart - channel number
Returns:  None
**************************************************************************/
int UART_getTail(int type, int uart){
	switch (uart){
		case 0:
			if (type){
				return UART_TxTail;
...
Purpose:  Receive a single byte from the receive buffer
Input:    Index of byte in buffer
Returns:  None
**************************************************************************/
uint8_t uart_get_byte(int index){
	if (index < UART_RX0_BUFFER_SIZE) {
		return UART_RxBuf[index];
	}
